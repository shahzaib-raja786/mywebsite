  ...
  return (
    <header className="w-full bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 text-white fixed top-0 left-0 z-50 shadow-md">
      <div className="max-w-7xl mx-auto flex justify-between items-center p-4">
        ...
        <nav className={`${menuOpen ? 'block' : 'hidden'} sm:flex sm:items-center sm:space-x-8`}>
          <Link 
            to="/" 
      ...
      {menuOpen && (
        <div className="sm:hidden bg-black text-white p-4 space-y-2">
          <Link to="/" className="block text-lg hover:text-yellow-300">Home</Link>
          <Link to="/about" className="block text-lg hover:text-yellow-300">About</Link>
          <Link to="/services" className="block text-lg hover:text-yellow-300">Services</Link>
        </div>
      )}
    </header>
  );
